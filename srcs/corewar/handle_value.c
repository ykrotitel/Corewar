...
/*   Updated: 2020/12/11 20:11:14 by lmittie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int32_t	get_arg(uint8_t arg_type, int32_t *pos, const uint8_t *arena)
{
	int32_t arg;
...
void	place_value(int32_t arg, int32_t pos, t_carriage *car, t_data *data)
{
	size_t size;
	...
	while (size--)
	{
		...
		data->code_color[get_pos(pos + size)] = car->color_code;
		arg >>= (int32_t)8;
	}
}
